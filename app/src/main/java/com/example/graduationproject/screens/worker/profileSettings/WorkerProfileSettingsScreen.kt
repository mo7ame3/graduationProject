package com.example.graduationproject.screens.worker.profileSettings
...
@Composable
fun WorkerProfileSettingsScreen(navController: NavController) {
    Scaffold(topBar = {
        TopAppBar(title = "") {
        ...
        val valid = if (isChanePassword.value) {
            (nameEdit.value.isNotBlank() && phoneEditNotError.value && addressEdit.value.isNotBlank()
                    && oldPasswordEditNotError.value
                    && newPasswordEditNotError.value
                    && confirmPasswordEditNotError.value)
        } else {
            (nameEdit.value.isNotBlank() && phoneEditNotError.value && addressEdit.value.isNotBlank())
        }
